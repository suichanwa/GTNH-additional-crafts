package com.myname.mymodid.compat.thaumicboots;
...
public final class ThaumicBootsTuning {
    ...
    public static final String TAG_AXIS_VERTICAL = "gtnhac_axis_vertical";
    private static final double DEFAULT_AXIS_MULTIPLIER = 1.0D;
    private static final double MIN_AXIS_MULTIPLIER = 0.0D;
    private static final double MAX_AXIS_MULTIPLIER = 3.0D;
    ...
    public static double getVerticalMultiplier(ItemStack bootsStack) {
        return getAxisMultiplier(bootsStack, TAG_AXIS_VERTICAL);
    }
    ...
    public static void setVerticalMultiplier(ItemStack bootsStack, double value) {
        setAxisMultiplier(bootsStack, TAG_AXIS_VERTICAL, value);
    }
    ...
    public static void applyAxisMultipliers(EntityPlayer player, ItemStack bootsStack) {
        ...
        double verticalMultiplier = getVerticalMultiplier(bootsStack);

        if (isNearly(forwardMultiplier, DEFAULT_AXIS_MULTIPLIER) && isNearly(strafeMultiplier, DEFAULT_AXIS_MULTIPLIER)
            && isNearly(verticalMultiplier, DEFAULT_AXIS_MULTIPLIER)) {
            return;
        }
        ...
        if (!player.onGround && Math.abs(player.motionY) > 0.0001D) {
