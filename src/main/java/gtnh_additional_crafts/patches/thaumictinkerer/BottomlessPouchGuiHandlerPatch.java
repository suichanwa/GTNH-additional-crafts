package gtnh_additional_crafts.patches.thaumictinkerer;
...
public class BottomlessPouchGuiHandlerPatch implements IGuiHandler {
    ...
    public Object getServerGuiElement(int id, EntityPlayer player, World world, int sourceType, int slot, int z) {
        ...
        ItemStack pouchStack = BottomlessPouchAccessPatch.getStack(player, sourceType, slot);
        if (!BottomlessPouchAccessPatch.isBottomlessPouch(pouchStack)) {
            return null;
        }
    ...
    private int getContainerSlotIndex(int sourceType, int slot) {
        if (sourceType != BottomlessPouchAccessPatch.SOURCE_INVENTORY) {
            return -1;
        }
        if (slot < 9) {
